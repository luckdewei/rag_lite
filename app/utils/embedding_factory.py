    ...
    def create_embeddings():
        ...
        try:
            ...
            # 如果提供商为 ollama
                ...
                # 记录日志
                logger.info(
                    f"创建 Ollama Embeddings: {model_name}, base_url: {base_url}"
                )

            else:
                # 未知的提供商，警告日志，使用默认 huggingface
                logger.warning(
                    f"未知的 Embedding 提供商: {provider}，使用默认的 HuggingFace"
                )
                embeddings = HuggingFaceEmbeddings(
            ...
            # 失败时回退到默认模型并记录警告
            logger.warning(
                f"回退到默认 HuggingFace 模型: {EmbeddingFactory.DEFAULT_MODEL_NAME}"
            )
